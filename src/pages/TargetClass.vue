<template>
    <div class="p-5">
        ...
        <transition-group class="grid grid-cols-4 gap-5 mt-5" tag="div">
            <StuCard v-for="item in filterStudents" :key="item[0]" :data="item"
...
export default {
    ...
    setup() {
        ...
        let filterStudents = computed(() => {
            let filteredStudents = allStudents.value ? allStudents.value.filter(student => {
                const fullName = `${student[1].nameStu} ${student[1].familyStu}`
                return fullName.includes(searchValue.value.trim()) && student[1].classStu === route.params.class;
            }) : [];

            return filteredStudents.sort((a, b) => {
                let familyA = a[1].familyStu;
                let familyB = b[1].familyStu;
